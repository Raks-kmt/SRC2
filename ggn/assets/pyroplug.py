...
async def copy_message_with_chat_id(client, sender, chat_id, message_id):
    ...
    try:
        ...
        caption = f"{final_caption}\n\n__**{custom_caption}**__" if custom_caption else f"{final_caption}\n\n__**[Team SPY](https://t.me/forcesub12)**__"
...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [Team SPY](https://t.me/forcesub12)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**__Bot made by [Team SPY](https://t.me/forcesub12)__**',
                upm,
                time.time()
            )
        )
...
async def get_msg(userbot, client, sender, edit_id, msg_link, i, file_n):
    ...
    if 't.me/c/'  in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            file = await userbot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[Team SPY](https://t.me/forcesub12)__**\n ", edit, time.time()))            # Retrieve user's custom renaming preference if set, default to '@devggn' otherwise
            if not file:
...
async def settings_command(event):
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/forcesub12")]
    ]
